<?php if(isset($_COOKIE['adminLogin'])) : ?>
<?php require_once "help_pages/header.php"; ?>

<?php 
	require_once "../../connect2/connect.php"; 
	require_once "../classes/Poem.php";
	$authorId = $_GET['author_id'];
	$poem = new Poem();
	$poems = $poem->getPoemsForAuthorId($authorId);
?>

<div class="myTable">
		<table cellspacing="0" cellpadding="0">
			<th style="width: 500px;">Вірші</th>
				<th colspan="2">
					<a href="poemInsert.php?author_id=<?=$authorId?>">
						<input type="submit" value="Добавити" style="width: 300px; margin: 10px; ">
					</a>
				</th>

			
				<?php foreach ($poems as $value) : ?>
					<tr>
					<td>
						<a href="showPoem.php?poem_id=<?=$value->getId()?>">
							<?=$value->getName()?>
						</a>
					</td>
					<input type="hidden" name="author_id" value="<?=$_GET['author_id']?>">
					<td>
						<a href="poemUpdate.php?author_id=<?=$authorId?>&poem_id=<?=$value->getId()?>">
							<input type="submit" value="Оновити">
						</a>
					</td>
					<td>
						<a href="forms/poemDeleteForm.php?author_id=<?=$authorId?>&poem_id=<?=$value->getId()?>">
							<input type="submit" name="delete_<?=$value->getId()?>" value="Видалити" onclick="return confirm('Ви дійсно хочете видалити це?');">
						</a>
					</td>
					</tr>	
				<?php endforeach; ?>
			
		</table>				
	
</div>



<?php require_once "help_pages/footer.php"; ?>

<?php else: ?>
	<?php header("Location: ../admin.php"); ?>
<?php endif; ?>

